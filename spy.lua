...
local function validateWebhook(url, callback)
    if not url or url == "" then
        callback(false, "URL is empty")
        return
    end
    if not url:match("^https://discord%.com/api/webhooks/") and
       not url:match("^https://discordapp%.com/api/webhooks/") and
       not url:match("^https://ptb%.discord%.com/api/webhooks/") and
       not url:match("^https://canary%.discord%.com/api/webhooks/") then
        callback(false, "Not a valid Discord webhook URL")
        return
    end
    if not request then
        callback(false, "request() not available")
        return
    end
    ...
    if ok and res then
        ...
        elseif res.StatusCode == 401 then
            ...
            callback(false, "Invalid webhook (Status: " .. tostring(res.StatusCode) .. ")")
        end
    else
        callback(false, "Request failed: " .. tostring(res))
    end
end
...
local function showWebhookPrompt(onComplete)
    ...
    box.Size = UDim2.new(0, 450, 0, 19 + 249)
    box.Position = UDim2.new(0.5, -225, 0.5, -134)
    box.BackgroundColor3 = Color3.fromRGB(53, 52, 55)
    ...
    titleLabel.Text = "Remotespy - by Vanish  |  Webhook Setup"
    ...
    skipBtn.Size = UDim2.new(0, 19, 0, 19)
    skipBtn.Position = UDim2.new(1, -19, 0, 0)
    skipBtn.BackgroundColor3 = Color3.fromRGB(37, 35, 38)
    skipBtn.BorderSizePixel = 0
    skipBtn.Font = Enum.Font.SourceSans
    skipBtn.Text = ""
    skipBtn.TextColor3 = Color3.new(0, 0, 0)
    skipBtn.TextSize = 14
    skipBtn.AutoButtonColor = false
    skipBtn.Parent = topbar

    local skipIcon = Instance.new("ImageLabel")
    skipIcon.Size = UDim2.new(0, 9, 0, 9)
    skipIcon.Position = UDim2.new(0, 5, 0, 5)
    skipIcon.BackgroundTransparency = 1
    skipIcon.Image = "http://www.roblox.com/asset/?id=5597086202"
    skipIcon.Parent = skipBtn

    local content = Instance.new("Frame")
    content.Size = UDim2.new(1, 0, 1, -19)
    content.Position = UDim2.new(0, 0, 0, 19)
    content.BackgroundColor3 = Color3.fromRGB(37, 36, 38)
    ...
    descLabel.Text = "Enter your Discord Webhook URL to enable remote logging:"
    ...
    inputBg.Position = UDim2.new(0, 10, 0, 48)
    inputBg.BackgroundColor3 = Color3.fromRGB(26, 25, 27)
    ...
    statusLabel.Position = UDim2.new(0, 10, 0, 86)
    ...
    local function makeButton(text, xPos, w, color)
    ...
    local validateBtn = makeButton("Validate & Start", 10, 130, Color3.fromRGB(99, 86, 245))
    local noWebhookBtn = makeButton("Skip (no logging)", 150, 130, Color3.fromRGB(60, 60, 63))

    local validating = false

    skipBtn.MouseEnter:Connect(function()
        TweenService:Create(skipBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 60, 60)}):Play()
    end)
    skipBtn.MouseLeave:Connect(function()
        TweenService:Create(skipBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(37, 35, 38)}):Play()
    end)
    ...
    noWebhookBtn.MouseButton1Click:Connect(function()
        URL = ""
        promptGui:Destroy()
        onComplete()
    end)
...
function makeToolTip(enable, text)
    if enable and text then
        ...
        connections["ToolTip"] = RunService.RenderStepped:Connect(function()
            ...
            elseif bottomRight.X > ViewportSizeX then topLeft = Vector2.new(ViewportSizeX - ToolTip.AbsoluteSize.X, topLeft.Y) end
            if topLeft.Y < 0 then topLeft = Vector2.new(topLeft.X, 0)
            elseif bottomRight.Y > ViewportSizeY - 35 then topLeft = Vector2.new(topLeft.X, ViewportSizeY - ToolTip.AbsoluteSize.Y - 35) end
    ...
    spawn(function()
        pcall(function()
            ...
            pcall(function()
                ...
                if #argsString > 800 then argsString = argsString:sub(1, 800) .. "... [TRUNCATED]" end
            end)
            pcall(function()
            ...
            pcall(function()
                ...
                if #generatedScript > 1500 then generatedScript = generatedScript:sub(1, 1500) .. "\n... [TRUNCATED]" end
            end)
            ...
            local webhookData = {
                embeds = {{
                    ...
                    color = 16777215,
                    fields = {
                        { name = "Remote Name", value = remote.Name or "Unknown", inline = true },
                        { name = "Type", value = type == "event" and "RemoteEvent" or "RemoteFunction", inline = true },
                        { name = "Remote Path", value = "```lua\n" .. remotePath .. "\n```", inline = false },
                        { name = "Calling Script", value = "```lua\n" .. callingScriptInfo .. "\n```", inline = false },
                        { name = "Arguments", value = "```lua\n" .. argsString .. "\n```", inline = false },
                        { name = "Generated Script", value = "```lua\n" .. generatedScript .. "\n```", inline = false }
                    },
                    timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
                    footer = { text = "Remote Logged | Debug ID: " .. (data.id or "N/A") }
                }}
            }
            request({
                Url = URL,
            ...
            if configs.advancedinfo then
                ...
                elseif getconnections then
                    ...
                    for i,v in next, getconnections(Remote.OnClientEvent) do
                        info["advancedinfo"]["OnClientEvents"][i] = { Function = v.Function or "N/A", State = v.State or "N/A" }
                    end
                end
            end
